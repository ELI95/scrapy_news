...
class NewsSpider(Spider):
    ...
    def parse(self, response):
        if 'login' in response.url:
            ...
            items = get_items(news_items_a_none, 'wallstreetcn_a_stock_channel', 'general', crawl_number=10)
            items.extend(get_items(news_items_a_key, 'wallstreetcn_a_stock_channel', 'key', crawl_number=5))
            items.extend(get_items(news_items_us_none, 'wallstreetcn_us_stock_channel', 'general', crawl_number=10))
            items.extend(get_items(news_items_us_key, 'wallstreetcn_us_stock_channel', 'key',  crawl_number=5))
            ...
            items = get_items(news_items_a_none, 'sina_news_10', 'general', crawl_number=10)
            items.extend(get_items(news_items_a_key, 'sina_news_10', 'key', crawl_number=5))
